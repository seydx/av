...
Napi::Value FormatContext::OpenOutput(const Napi::CallbackInfo& info) {
  ...
  if (!(ctx->oformat->flags & AVFMT_NOFILE)) {
    int ret = avio_open(&ctx->pb, ctx->url, AVIO_FLAG_WRITE);
    if (ret < 0) {
      char errbuf[AV_ERROR_MAX_STRING_SIZE];
      av_strerror(ret, errbuf, sizeof(errbuf));
      Napi::Error::New(env, std::string("Failed to open output file: ") + errbuf).ThrowAsJavaScriptException();
      return env.Undefined();
    }
    return Napi::Number::New(env, ret);
  }
...
Napi::Value FormatContext::CloseOutput(const Napi::CallbackInfo& info) {
  ...
  // avio_closep sets pb to nullptr automatically
  // Note: We don't free the context here, that's done by freeContext
  // Close pb if it exists and the format requires file I/O
  // OR if it's a custom IO context (no oformat check needed for custom IO)
  if (ctx->pb) {
    if (!ctx->oformat || !(ctx->oformat->flags & AVFMT_NOFILE)) {
